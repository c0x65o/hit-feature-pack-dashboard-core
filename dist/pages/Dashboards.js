...
export function Dashboards(props = {}) {
    ...
    const queryMetrics = React.useCallback(async () => {
        ...
        const apiPies = widgets.filter((w) => w?.kind === 'api_pie');
        if (apiPies.length) {
        ...
        const apiTableWidgets = widgets.filter((w) => w?.kind === 'api_table');
